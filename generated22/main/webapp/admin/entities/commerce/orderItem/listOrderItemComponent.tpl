...
<ui:composition xmlns:ui="http://java.sun.com/jsf/facelets" xmlns="http://www.w3.org/1999/xhtml" xmlns:c="http://java.sun.com/jstl/core" xmlns:uvs="http://www.univeris.com/jsf/ewms/8.x" xmlns:f="http://java.sun.com/jsf/core" xmlns:h="http://java.sun.com/jsf/html" xmlns:s="http://jboss.com/products/seam/taglib" xmlns:a="http://richfaces.org/a4j" xmlns:p="http://primefaces.prime.com.tr/ui" xmlns:rich="http://richfaces.org/rich">  
  <uvs:secured fsaid="20000" access-type="8"> 
    <h:panelGroup id="panelProfiles" layout="block"> 
      ...
      <rich:dataTable id="recordsTable" rows="10" value="#{orderItemAction.all}" var="orderItem" style="border-right:solid 1px #c4c0c9;" headerClass="lj" columnClasses="autoWrapText bottomBorderOnly, autoWrapText bottomBorderOnly, autoWrapText bottomBorderOnly, autoWrapText bottomBorderOnly"> 
        <rich:column sortBy="#{record.qty}"> 
          <f:facet name="header">#{uvs:label('QTY')}</f:facet>  
          <h:outputText value="#{orderItem.qty}" escape="false"/> 
        </rich:column>  
        <rich:column sortBy="#{record.product}"> 
          <f:facet name="header">#{uvs:label('PRODUCT')}</f:facet>  
        ...
        <rich:column sortBy="#{record.customerOrder}"> 
          <f:facet name="header">#{uvs:label('CUSTOMERORDER')}</f:facet>  
