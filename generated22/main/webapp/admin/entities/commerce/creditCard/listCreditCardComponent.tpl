...
<ui:composition xmlns:ui="http://java.sun.com/jsf/facelets" xmlns="http://www.w3.org/1999/xhtml" xmlns:c="http://java.sun.com/jstl/core" xmlns:uvs="http://www.univeris.com/jsf/ewms/8.x" xmlns:f="http://java.sun.com/jsf/core" xmlns:h="http://java.sun.com/jsf/html" xmlns:s="http://jboss.com/products/seam/taglib" xmlns:a="http://richfaces.org/a4j" xmlns:p="http://primefaces.prime.com.tr/ui" xmlns:rich="http://richfaces.org/rich">  
  <uvs:secured fsaid="20000" access-type="8"> 
    <h:panelGroup id="panelProfiles" layout="block"> 
      ...
      <rich:dataTable id="recordsTable" rows="10" value="#{creditCardAction.all}" var="creditCard" style="border-right:solid 1px #c4c0c9;" headerClass="lj" columnClasses="autoWrapText bottomBorderOnly, autoWrapText bottomBorderOnly, autoWrapText bottomBorderOnly, autoWrapText bottomBorderOnly"> 
        <rich:column sortBy="#{record.accountAddress}"> 
          <f:facet name="header">#{uvs:label('ACCOUNTADDRESS')}</f:facet>  
          <h:outputText value="#{creditCard.accountAddress}" escape="false"/> 
        </rich:column>  
        <rich:column sortBy="#{record.ccNumber}"> 
          <f:facet name="header">#{uvs:label('CCNUMBER')}</f:facet>  
          <h:outputText value="#{creditCard.ccNumber}" escape="false"/> 
        </rich:column>  
        <rich:column sortBy="#{record.expiry}"> 
          <f:facet name="header">#{uvs:label('EXPIRY')}</f:facet>  
          <h:outputText value="#{creditCard.expiry}"> 
