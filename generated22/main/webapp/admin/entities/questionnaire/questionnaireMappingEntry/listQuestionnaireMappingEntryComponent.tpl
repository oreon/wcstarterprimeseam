...
<ui:composition xmlns:ui="http://java.sun.com/jsf/facelets" xmlns="http://www.w3.org/1999/xhtml" xmlns:c="http://java.sun.com/jstl/core" xmlns:uvs="http://www.univeris.com/jsf/ewms/8.x" xmlns:f="http://java.sun.com/jsf/core" xmlns:h="http://java.sun.com/jsf/html" xmlns:s="http://jboss.com/products/seam/taglib" xmlns:a="http://richfaces.org/a4j" xmlns:p="http://primefaces.prime.com.tr/ui" xmlns:rich="http://richfaces.org/rich">  
  <uvs:secured fsaid="20000" access-type="8"> 
    <h:panelGroup id="panelProfiles" layout="block"> 
      ...
      <rich:dataTable id="recordsTable" rows="10" value="#{questionnaireMappingEntryAction.all}" var="questionnaireMappingEntry" style="border-right:solid 1px #c4c0c9;" headerClass="lj" columnClasses="autoWrapText bottomBorderOnly, autoWrapText bottomBorderOnly, autoWrapText bottomBorderOnly, autoWrapText bottomBorderOnly"> 
        <rich:column sortBy="#{record.min}"> 
          <f:facet name="header">#{uvs:label('MIN')}</f:facet>  
          <h:outputText value="#{questionnaireMappingEntry.min}" escape="false"/> 
        </rich:column>  
        <rich:column sortBy="#{record.max}"> 
          <f:facet name="header">#{uvs:label('MAX')}</f:facet>  
          <h:outputText value="#{questionnaireMappingEntry.max}" escape="false"/> 
        </rich:column>  
        <rich:column sortBy="#{record.outcome}"> 
          <f:facet name="header">#{uvs:label('OUTCOME')}</f:facet>  
          <h:outputText value="#{questionnaireMappingEntry.outcome}" escape="false"/> 
        </rich:column>  
        <rich:column> 
          <f:facet name="header"></f:facet>  
          <uvs:secured fsaid="20000" access-type="2"> 
            <h:commandLink action="#{questionnaireMappingEntryAction.delete(profile.code)}" title="#{uvs:label('DELETE')}" onmouseout="elem('img_#{profile.code}').src='#{uvs:nonJsfServletPath()}/files/cob/del_inactive.png'" onmouseover="elem('img_#{profile.code}').src='#{uvs:nonJsfServletPath()}/files/cob/del_active.png'" onclick="if(confirm(&quot;#{uvs:label(&quot;CONFIRM_DELETE_CLIENT_PROFILE_FROM_LIST&quot;)}&quot;)) return true; else return false;"> 
